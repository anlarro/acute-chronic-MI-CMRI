...
#########################33
#Cargamos datos
lge=read.csv('lge.csv')
Area_lge<-lge[,1]
lge<-lge[,-1]
lge<-lge[,!apply(is.na(lge),2,any)]

input=cbind(lge[,ncol(lge)],lge[,-ncol(lge)])  #lge
names(input)[1]<-"CLASS"
...
#sweepModelosFisher<-list()
#rocsModelosFisher<-list()
#for (n in 1:length(modelos)){
#  ptm<-Sys.time()
#  cl <- makeCluster(8) #use 6 cores, ie for an 8-core machine
#  clusterEvalQ(cl, {library(caret); library(pROC)})
#  sweepModelosFisher[[n]]<-parLapply(cl,1:(ncol(input)-1),FeatSweepROC.wrap,rankFisher,input,modelos[n],grillas[[n]])
#  stopCluster(cl)
#  rocsModelosFisher[[n]]<-sapply(sweepModelosFisher[[n]], function(y) mean(sapply(y, function(x) x$ROC)))
#  print(paste('Fisher',n))
#  print(Sys.time()-ptm)
#}


#Plot profile SVM-RFE
tiff("profileLGE.tiff", width = 3200, height = 3200, units = 'px', res = 600, compression="lzw")
plot(seq(1,length(rocsModelosSVMRFE[[3]]),by=1),rocsModelosSVMRFE[[3]][seq(1,length(rocsModelosSVMRFE[[3]]),by=1)],type="l",pch=16,xlim=c(0,length(rocsModelosSVMRFE[[3]])),ylim=c(0.55, 1),yaxt="n",xaxt="n",xlab="Number of features",ylab="AUC (Repeated cross-validation)",main="LGE MRI",lwd=2,lty=1,col="blue")
grid()
...
################################33
#Unique ROC plot
tiff("rocLGE.tiff", width = 3200, height = 3200, units = 'px', res = 600, compression="lzw")
...
plot(perf,col="red",lwd=2,lty=5,avg="threshold",ylab="Sensitivity",xlab="1-Specificity",main="LGE MRI") #curva ROC
...
#######################################################
#Scatter Plot
tiff("scatterLGE.tiff", width = 3200, height = 3200, units = 'px', res = 600, compression="lzw")
